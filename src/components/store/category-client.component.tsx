...
export default function CategoryClient({
  ...
  // Sorting needs the search endpoint (categoryId); only usable if we know the id.
  const canSort = categoryId != null;
      ...
      <section className="relative overflow-hidden bg-ink">
        ...
        <div className="relative mx-auto max-w-[1280px] px-4 py-10 md:py-14 sm:px-6 lg:px-8">
          ...
          <p className="mt-3 max-w-[560px] text-[16px] leading-[1.55] text-ink2-300">
            Explora nuestra selección de {categoryTitle.toLowerCase()} para
            proyectos de tecnología, redes y seguridad en Honduras.
          </p>
        </div>
      </section>
        ...
        {subcategories.length > 0 && (
          <div className="mb-10 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {subcategories.map((sub) => (
              <Link
                key={sub.id}
                href={`/tienda/${categorySlug}/${sub.slug}`}
                className="sb-card flex items-center gap-4 rounded-card border border-line bg-white px-5 py-4 shadow-card transition-shadow"
              >
                <span className="flex h-12 w-12 flex-none items-center justify-center rounded-[12px] bg-accent-soft text-accent">
