        ...
        <div className='container flex gap-5'>
            {
                products != null ? products.map((item:Product,key:number)=>{
                    return(
                        <div key={key} className='w-full mt-2 border hover:shadow-2xl '>
                            ...
                            <div className='col-span-2 pt-2 pl-2'>
                                <Link href={`/shop/category/${item.subcategory.category.slug}/${item.subcategory.slug}/${item.slug}`}>
                                    <h6 className='font-semibold text-center text-ellipsis text-sm text-[#191919]'>{item.name}</h6>
                                </Link>
                                <div className='font-semibold text-center text-sm my-2 text-[#1C68E1]'>
